...
| Cambio en lo que el ACTUATOR expone o en quién lo alcanza | `keel-spring/src/scaffold/config.js` (`managementYaml`) + `src/scaffold/security.js` (`authorizeBlock`) + `src/scaffold/integration-tests.js` (`actuatorMetric`) + `test/corrida-fixes.test.js`. Son tres artefactos que tienen que decir lo mismo | `telemetría` |
...
| Cambio en la telemetría (`telemetry: otel`) | Es elección de **stack**: `TELEMETRY`/`TELEMETRY_INFRA` en `src/lib/stack-catalog.js` + `stack-config.js` + flag `--telemetry`. El Java en `src/scaffold/telemetry.js` y compañía; `test/telemetry.test.js` + `java-syntax` + `compile-check --telemetry=otel` + `npm run telemetry-check` | `telemetría` |
