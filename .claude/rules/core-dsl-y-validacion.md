...
## Nuevo parámetro de despliegue del servicio (`service.parameters`, DSL 2.15)

`keel-core/assets/core/schema/service.schema.json` + `docs/dsl/service.md` + la fila del historial en `dsl-reference.md` + `CHK-SERVICE-PARAM-UNBACKED` en `crossrefs.js` + `keel-spring/src/lib/model.js` (`buildService`, que resuelve el gradiente una sola vez) + `src/scaffold/service-parameters.js` + `config.js` (el fragmento por perfil) + `deploy.js` (`.env`). **Qué es y qué no**: un valor único para todo el servicio que no es dato de negocio (nadie lo guarda), ni entrada de ninguna operación (no lo manda el cliente), ni infraestructura (no lo elige el stack) — la moneda en la que opera el catálogo, la zona horaria con la que se corta un día. Salió de la corrida `catalog` 2 (2026-09-21) y **no lo reportó ningún agente**, igual que el hueco de método de la corrida anterior: lo delató el diff de una regeneración limpia. El diseño lo decía en una `rule` del dominio y el agente se inventó las cuatro cosas que cuelgan de él —la clave de configuración, la variable de entorno, los perfiles en los que aparece y si el arranque falla cuando falta—, ninguna escrita en ninguna parte. De un parámetro declarado build emite **cuatro artefactos que tienen que decir lo mismo** (fragmento con su gradiente, record de dominio con sus guardas, su `@Configuration` y la variable del despliegue) y por eso el gradiente se resuelve en `buildService` y no en cada emisor. Las cotas se comprueban en el **constructor del record**, no con anotaciones de Spring: un parámetro mal puesto tiene que morir en el ARRANQUE, no en la primera petición que lo use. `requiredInProduction` por defecto `true` porque un default silencioso en producción hace que el servicio opere con un valor que nadie eligió.

## Unicidad ACOTADA A LA COLECCIÓN sin `code` (`CHK-PERSIST-CHILD-UNIQUE-CODE`)

Hermano de `CHK-PERSIST-CONDITIONAL-UNIQUE-CODE` por el otro lado, y el mismo hueco en su **segunda corrida seguida**, que es lo que lo hizo id obligatorio. Un índice `unique` de una entidad INTERNA que incluye la relación a su raíz («dos imágenes del mismo producto no comparten posición») no dice «ya existe un X con esos campos»: el padre es implícito en la petición y el otro miembro lo REPARTE el servicio entre toda la colección, así que pedir un valor ocupado es legal y lo único que rompe la constraint es el estado intermedio del reparto o una carrera. `keel-core/src/lib/crossrefs.js` + `checks.js` + `keel-spring/src/scaffold/controllers.js`, donde `raceOnlyConstraint` devuelve ahora el **motivo** (`'computed' | 'collection' | null`) porque de él cuelgan el mensaje y el comentario. **Dos trampas que costaron un ciclo cada una**: (a) la familia de la condición no puede ser solo el token —`(^|_)MAIN(_|$)` casaba con `MAIN_IMAGE_REQUIRED`, que significa lo CONTRARIO, y el generador mandaba ese error al violarse el índice; exige las dos mitades, el token y una palabra que diga que ya hay uno; (b) `errorsWrittenBy` buscaba operaciones del grupo de la HIJA, y una hija no tiene operaciones propias, así que devolvía siempre cero y el aviso pedía declarar un `code` que el generador era incapaz de recoger — el acotado incluye la raíz, y ahí se desactiva la pasada laxa («cualquier 409 que diga ya existe») o el `SKU_ALREADY_EXISTS` del producto acabaría siendo el error de la unicidad de su imagen.
