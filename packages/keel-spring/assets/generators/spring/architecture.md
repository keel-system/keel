...
- **`infrastructure`**: implementa los puertos y conecta con el mundo exterior — persistencia (JPA), mensajería (broker), HTTP saliente, storage de archivos, seguridad y la capa REST entrante. Es la única capa que conoce el framework y las tecnologías concretas del stack.
  - `configurations/usecase` (`UseCaseMediator`): abre la transacción (readOnly para `Query`, de escritura para `Command`/`ReturningCommand`) y despacha al handler correspondiente; es el único punto por el que pasa una operación. Con **una** excepción declarada, `dispatchWithoutTransaction`: el barrido de una reconciliación (una operación con `schedule` que el diseño enlaza con `reconciledBy`) corre **sin** transacción abarcadora, porque llama a un proveedor en medio de su trabajo y su garantía es un orden de commits, no una transacción — el reclamo tiene que confirmarse antes de la llamada y la llamada tiene que quedar fuera de toda transacción. Sigue pasando por el mediator: lo que cambia es qué abre, no por dónde entra.
