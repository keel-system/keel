...
- `lifecycle` se protege con un guard **privado** `transitionTo(target)` (mapa de transiciones + `InvalidStateTransitionException`, code `INVALID_STATE_TRANSITION` → HTTP 409); los métodos semánticos por transición los añade el agente (build deja un TODO por transición declarada, **nombrando la operación que la ejecuta** cuando el diseño lo declara en `use-cases.<op>.transitions`) y son la única vía para llamar al guard. Ese nombre no es decorativo: dice a qué handler pertenece el método semántico, y sin él el camino corto es mutar el estado desde fuera del agregado.
...
| `input`/`output` `{ entity: X }` | DTOs derivados de la entidad; en input quedan fuera `generated`/`computed` y **el campo del `lifecycle`** (lo mueve la máquina de estados del dominio —el `default` al crear, las `transitions` después—, no el cliente: ofrecerlo en el DTO invitaba a exigirlo con `@NotNull` y a que el camino feliz de la creación respondiera 400); en output quedan fuera `sensitive` y los campos de `exclude`. Un campo con `default` que **no** gobierna el lifecycle sí entra —puede ser una preferencia que el cliente fija— pero sin exigir presencia, y build lo enumera en un warning: si era estado interno, se saca con `input.exclude` en el diseño, no ignorándolo en el handler. Las **relaciones entran por defecto**: una referencia a otro agregado como `<relación>Id` (UUID) —salvo que el output la marque con `embed`, en cuyo caso el DTO lleva el `<Raíz>RefDto` que **produce el `<Raíz>RefResolver` que build inyecta en el handler**, siempre por lote: cómo usarlo, en [read-composition](read-composition.md)— y una entidad hija como su propio `<Hija>Dto` (`List<…>` si es `one-to-many`), que build genera. Ese `<Hija>Dto` **lleva el id de su raíz** (`<relación>Id`), porque el mismo record se devuelve anidado dentro del payload del padre y también SUELTO cuando una operación declara `output: { entity: <Hija> }` —y ahí, sin el id del padre, la respuesta no dice a qué padre pertenece—. En el dominio la hija no guarda puntero a su raíz (la FK vive en persistencia), así que ese id **entra por parámetro** del método del mapper (`to<X>Dto(<Hija> entity, UUID <relación>Id)`), igual que un `RefDto`: cuando la hija va anidada lo pasa el propio mapper del padre, y cuando va suelta lo pasa **el handler** — el compilador no te deja olvidarlo. Un `exclude` con **dot-path** (`lines.costPrice`, `address.zip`) recorta el DTO **anidado**: build genera la hija completa y señala cada ruta con un warning — ese warning es la señal de trabajo del agente. En el **input** las hijas no entran (se gestionan por sus propias operaciones); si un flujo las recibe anidadas, build lo avisa y lo modela el agente |
...
| `idempotency: { keySource: payload-field, keyField }` | La clave es **un campo del command**, así que llega por igual desde HTTP y desde el broker. Build no genera `IdempotencyContext` ni el filtro. Y hay dos esqueletos según lo que ya la guarde: **si `keyField` participa en la `naturalKey`** del agregado, build **no genera nada** —esa constraint ES la guarda, permanente y común a todas las puertas— y el handler busca por la clave natural antes de insertar, devolviendo el recurso existente si ya está; **si no participa**, build genera el almacén de siempre, tecleado por ese campo. La nota del handler dice cuál de los dos toca — no lo deduzcas |
...
| `schedule: { cron }` | `@Scheduled(cron = ...)` que despacha el mensaje de la operación vía `UseCaseMediator`; sin endpoint. Si además es el `reconciledBy` de una activación (un barrido), el despacho es `dispatchWithoutTransaction`: sin transacción abarcadora, porque la llamada al proveedor va en medio — ver `dependencies.md § El orden dentro del barrido` |
...
| `EventEnvelope.metadata.correlationId` | Sale de `CorrelationContext` (`infrastructure/correlation`), que puebla `CorrelationFilter` en cada request HTTP (header `X-Correlation-Id`, generado si no viene, devuelto en la respuesta) y el listener en cada mensaje con `CorrelationContext.runWith(envelope.metadata(), ...)`. También llega al `ErrorResponse` y a cada línea de log (`logging.pattern.correlation`) |
| `EventEnvelope.metadata.traceparent` | Contexto de traza W3C. Con telemetría (`telemetry: otel`) lo estampa `EventEnvelope.of(...)` desde el span activo, el relay del outbox publica dentro de él y `CorrelationContext.runWith(envelope.metadata(), ...)` lo continúa al consumir; sin telemetría viaja a `null`. Ver `conventions/observability.md` |
