...
└── src/
    ...
    │   │   ├── support/         # solo con embed: <Raíz>RefResolver (resuelve el <Raíz>RefDto POR LOTE)
    │   │   └── projection/      # solo con capa dependencies: <E>Projector + <E>Reader
    │   ├── domain/
    ...
    │       ├── configurations/cache/    # (si alguna operación declara cache) CacheConfig: CacheManager, TTL y constante por caché, JSON con JavaTimeModule, degradación a miss
...
`keel-spring build` genera de forma determinista (re-ejecutable: sin banderas solo añade archivos nuevos, `--refresh --prune` pone al día lo suyo sin pisar lo del agente, `--force` sobrescribe):
...
- `application/`: contratos CQRS, por operación su record mensaje (`XxxCommand` con Bean Validation — es el body HTTP — o `XxxQuery`) y su handler stub `@ApplicationComponent` en `usecases/` con `// TODO (agente)` citando reglas, errores y políticas (inyecta el **puerto** de dominio y el mapper, nunca JPA); `XxxResponseDto` + el `<Hija>Dto` de cada entidad hija proyectada + el `<Raíz>RefDto` de cada relación marcada con `embed` + `PagedResponse` (+ `FileUpload` si alguna entrada trae un campo `file`); `<Entidad>ApplicationMapper` con mapeo campo a campo, incluidas las hijas. Con `embed`, además `support/` con el `<Raíz>RefResolver` de cada agregado embebido, que produce su `RefDto` **por lote** (`resolve(Collection)` → `Map` indexado por id, sobre el `findAllById` que el puerto de esa raíz gana solo en este caso) y se inyecta en los handlers que lo necesitan — sin él el handler no compilaría, porque el mapper exige el `RefDto` por parámetro y el agregado solo guarda el id ajeno; el criterio de uso está en `read-composition.md`. Con capa `dependencies`, además `projection/` con el `<E>Projector` (escribe la copia local de un dato ajeno; **es el único sitio desde el que se escribe**) y el `<E>Reader` (la lee aplicando la política `onMiss`); ambos con `@ApplicationComponent`, sin Spring, dentro de la transacción que abre el mediator.
...
- `infrastructure/messaging/idempotency/` (con `subscriptions` + capa `persistence`): `ProcessedEventJpa` (tabla `processed_event`, PK compuesta handler+evento), su repositorio e `IdempotencyGuard` (registro en transacción propia, purga por cron parametrizada). Es la cara simétrica del outbox: este garantiza que un evento no se pierde, aquel que reentregarlo no lo procese dos veces. El listener del agente lo **usa**; no escribe otro mecanismo.
