...
- **Regeneración segura**: re-ejecutar `build` solo añade archivos nuevos; lo implementado por el agente no se pisa. Con `--refresh --prune` pone al día lo que es suyo, retira lo que el diseño quitó y nadie tocó, y deja `build/keel-refresh/EVOLUTION.md` para que `/keel-generate-spring` entre en modo evolución (ver `orchestration.md § Modo evolución`). Con `--force` se sobrescribe todo lo generado.
...
| `conventions/observability.md` | **Solo con telemetría** (`telemetry: otel`): el patrón colector, qué instrumenta build, perfiles y muestreo, la traza a través de los eventos y las reglas para no romperla ni filtrar datos. La cardinalidad de las etiquetas la vigila `infra/check-telemetry.sh` |
