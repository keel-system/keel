...
export function checkSupportedFeatures(manifest, layers) {
  ...
  if (reconciled.length > 0) {
    warnings.push(
      `dependencies.activations.reconciledBy (${reconciled.join(', ')}): build genera el @Scheduled con su cron, la nota de qué barrer y los tres números del barrido en parameters/<perfil>/reconciliation.yaml (el umbral sale del diseño, 'unansweredAfterSeconds'; la caducidad del reclamo y el tamaño de lote los pone el generador), pero la consulta de candidatos y la decisión (reintentar el encargo o compensarlo) las escribe el agente, que debe LEER esos parámetros en vez de elegir constantes. Y SÍ tiene escenario: el cron no se llama desde fuera, pero su condición de entrada se crea con ageForReconciliation(<activación>, id) —el arnés envejece la marca de espera de esa fila y el cron dispara solo—, así que el barrido se puntúa como cualquier otro flujo. Se envejece LA FILA y no el umbral: unansweredAfterSeconds es global y bajarlo se llevaría por delante las filas de los demás escenarios, que están esperando su desenlace normal. Además lo cubre infra/check-idempotency.sh en estático.`
    );
  }
