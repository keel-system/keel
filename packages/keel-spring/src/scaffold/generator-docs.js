...
1. **Fase 1 — en paralelo** (tres Task en un único mensaje):
   ...
   Los tres arrancan a la vez porque todos sus insumos ya están en disco: no hay arista entre ellos. Que el autor de las pruebas **nunca vea el código terminado** es justo lo que impide que un test se acomode a lo que el código hace en vez de a lo que el \`Then\` dice; y el source set las compila sin \`src/main/java\`, así que el paralelismo es real. Espera a los tres. Cada agente cierra su reporte con un bloque estructurado (\`status\`, \`blockers\`, \`failures\`…): el gating se decide sobre esos campos. Sin docker/podman (\`infra status: PENDIENTE\`) → **detente**: sin infraestructura no hay validación end-to-end y, al no haber suite unitaria, la generación queda sin red de seguridad; reporta el código como compilado pero NO validado e indica cómo levantar la infraestructura. Infra KO corregible → relanza \`keel-spring-infra\` una vez con el diagnóstico. \`code\` con \`compiles: false\` → relanza \`keel-spring-code\` pasándole sus \`failures\` (máx. 2 ciclos). \`tests\` en KO **por causa propia** → relánzalo; si su KO viene de \`src/main/java\` o de contención de locks de Gradle, no es suyo: se reevalúa al cerrar el gate de \`code\`. \`blockers\` no vacío en cualquiera → detente y repórtalo al usuario.
2. **Fase 2 — validación funcional (el gate de la generación: exige el 100% de los escenarios en OK).** Solo con los tres en OK: lanza \`keel-spring-validate\` con la raíz \`.\` y el reporte de infraestructura. Ejecuta \`./gradlew integrationTest\` (la app la arranca JUnit contra la infra real; el reset por flujo lo hace cada clase en su \`@BeforeAll\`), compone la matriz desde el XML de JUnit y **arbitra** cada fallo contra el \`Then\` original con la evidencia de \`build/keel-failures/\`. El veredicto \`culprit\` decide a quién relanzar: \`code\` → ciclo \`keel-spring-code\` → \`keel-spring-validate\` pasándole **exactamente** sus \`failures\` (con el campo \`evidence\`: el agente abre el JSON crudo de \`build/keel-failures/\`, no el extracto, y lo hace antes de ejecutar nada porque una pasada nueva lo sobrescribe); \`test\` → relanza \`keel-spring-tests\` (corrige la prueba, no el servicio); \`design\` → detente y propón el cambio a los artefactos, nunca acomodes el código en silencio. El agente relanzado cierra verificando su fix con \`./gradlew integrationTest --tests '<ClaseAfectada>'\`, pero **ese verde no aprueba el escenario**: tras cualquier ciclo de fix se vuelve siempre a \`keel-spring-validate\` con la suite completa —única que ve una regresión en otro flujo y la única de la que sale la matriz—, nunca directamente a la fase 3. Si la tanda mezcla \`culprit: code\` y \`culprit: test\`, relánzalos **en serie** (primero \`code\`): ambos ejecutan Gradle sobre este directorio y \`resetState()\` vacía la misma base de datos, así que en paralelo se estorban. **Cómo contar los ciclos**: el cupo cuenta solo los ciclos código→validación por fallos puntuales (\`blocking: scoped\`) y **escala con el número de flujos \`FL-*\`** del diseño — hasta 10 flujos, 2 ciclos (tope duro 4); de 11 a 20, 3 (tope 5); más de 20, 4 (tope 6). No consumen cupo ni los ciclos que cerraron un **bloqueo sistémico** (\`blocking: systemic\`: una causa transversal —seguridad, arranque, infraestructura— que impedía ejercitar prácticamente cualquier escenario, y destrabarla es lo que hace visibles por primera vez los fallos finos) ni los \`culprit: test\`, que no dicen nada sobre el código. Alcanzado el tope que aplique, reporta la matriz y detente. Detalle completo en \`.claude/orchestration.md\`.
3. **Fase 3 — calidad${model.layersPresent.persistence ? ' + baseline de migraciones' : ''}.** Solo con **todos** los escenarios OK: lanza \`keel-spring-quality\` sobre la raíz \`.\`. Aplica solo cambios no-conductuales y cierra con \`./gradlew build -x test\` en verde **y \`./gradlew integrationTest\` al 100%** — la no-regresión la comprueba él mismo, ya no hace falta un nodo de re-validación; si reporta \`status: KO\` o \`scenarios: KO\`, revierte/reporta — nunca hagas commit con la compilación en rojo o un escenario fallando.${model.layersPresent.persistence ? ' El mismo agente produce el **baseline de migraciones** (`db/migration/V1__baseline_schema.sql`, exportado de las entidades ya finales con `infra/export-schema.sh`) y lo prueba arrancando con `PROFILE=local,migrations` sobre una BD sin esquema: exige `baseline: OK`, porque sin él el servicio compila y valida pero no es desplegable (en production Hibernate solo valida el esquema). `baseline: KO` → relánzalo una vez con su error exacto.' : ''} Consolida sus \`remaining\` en el resumen. Al terminar, baja la infraestructura (\`docker compose -f infra/docker-compose.yaml down\`, o \`podman compose\`).
4. **Guía de despliegue productivo.** Con los escenarios en 100% OK tras el pase de calidad y **antes** del commit, completa la sección \`## Despliegue en producción\` del \`README.md\` para que enumere todos los pasos y todos los parámetros necesarios para levantar el servidor en producción. El scaffolding dejó un baseline (pasos + tabla de parámetros obligatorios); tu trabajo es reconciliarlo con lo que los agentes realmente cablearon: si al implementar los adaptadores del stack (publishers/listeners del broker, adaptador de storage, auth saliente de los clientes HTTP) aparecieron parámetros nuevos, añádelos. Fuente de verdad: \`src/main/resources/parameters/production/*.yaml\` (todo \`\${VAR}\` sin default es obligatorio) + \`keel-stack.json\` + el código final. No inventes parámetros: si un \`\${VAR}\` no aparece en \`parameters/production/\`, no va en la guía.
...
- \`.claude/conventions/\` — mapeo DSL → código (\`mapping.md\`, síguelo estrictamente), estructura del proyecto (\`project-layout.md\`), sondeo y reset de infraestructura (\`infra-validation.md\`), traducción de los escenarios \`FL-*\` a pruebas de integración (\`integration-tests.md\`), auditoría de fidelidad al flujo (\`flow-fidelity.md\`), modelado del dominio (\`domain-modeling.md\`: agregados ricos, invariantes y reparto de la validación) y guías de handler (\`domain-services.md\`, \`virtual-threads.md\`).${techSkillsBullet}
