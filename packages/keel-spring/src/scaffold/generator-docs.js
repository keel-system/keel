...
// Conventions que solo se instalan con una elección de stack. Hoy, la de observabilidad: sin
// telemetría describiría piezas que el proyecto no tiene, y el agente las buscaría.
export const TELEMETRY_CONVENTIONS = ['observability.md'];
...
/**
 * El gate del formato de los value types escalares. Mismo reparto que el anterior sobre
 * otra pieza —build genera `<Tipo>Format`, el agente escribe la llamada—, y con la misma
 * razón para existir: su ausencia no rompe nada, el servicio acepta valores que el diseño
 * declara imposibles y ningún escenario que no lo mire lo delata.
 */
function qualityFormatGate(model) {
  if (!usesDomainGuardsCheck(model)) return '';
  return ' Incluye también `bash infra/check-domain-guards.sh` (familia `valueTypeFormat`): comprueba que todo campo tipado con un value type escalar con `pattern` tenga quien lo haga cumplir tras normalizar. No hay escenario `FL-*` que garantice esa cobertura —solo cubren los campos que alguien pensó en cubrir—, así que este es su único gate.';
}

/**
 * La higiene de los logs que escribe el agente (`infra/check-logging.sh`). Se genera siempre y,
 * a diferencia de los dos anteriores, NACE verde: veta una forma, no exige un uso. Por eso su
 * rojo solo puede venir de código del agente.
 */
function qualityLoggingGate() {
  return ' Y `bash infra/check-logging.sh` (familia `logging`): veta concatenar dentro de un log, loguear un objeto de entrada entero (su `toString()` lleva al backend de logs los datos de quien llama), un `log.error` desde `domain/` o `application/` (ahí un fallo se lanza, no se loguea) y un executor que no propaga el contexto. Nace verde —build no escribe ninguna de las dos formas—, así que un `KO` es siempre código nuevo: relánzalo con sus hallazgos exactos. Qué loguea ya build y qué puede añadir el agente está en `{{keel:docs}}/conventions/logging.md`.';
}

/**
 * La cardinalidad de las métricas (`infra/check-telemetry.sh`), solo con telemetría. Nace verde
 * como el de logs, y lo que veta no rompe nada visible: se ve en la factura del backend.
 */
function qualityTelemetryGate(model) {
  if (!usesTelemetry(model)) return '';
  return ' Y `bash infra/check-telemetry.sh` (familia `cardinality`): veta una etiqueta de métrica con una clave fuera del vocabulario de build. Es el único gate cuyo hallazgo no produce ningún síntoma —ni excepción, ni log, ni escenario rojo—: una etiqueta con un id dentro multiplica las series por el número de valores, y eso se ve en la factura del backend. La salida está en la propia regla: lo que identifica algo va al span con `addHighCardinalityKeyValue`, no a la métrica. Nace verde, así que un `KO` es siempre código nuevo.';
}
...
0. **Precondiciones.** \`specs/\` es un snapshot del diseño que \`keel-spring build\` ya validó (schemas, referencias cruzadas y frontera del generador) y que refresca en cada build: no lo revalides ni lo edites — el canónico es \`specs/${service.name}/\` del workspace de diseño, y un cambio funcional lo hace **el diseñador** allí y re-ejecuta \`keel-spring build specs/${service.name}\`. **Tú no editas ninguno de los dos**: ni este snapshot ni el workspace (aunque esté a dos directorios de aquí). Un escenario que contradice al diseño es un \`culprit: design\`: se **propone** en \`design-gaps.yaml\` y en el informe, no se aplica — corregido por quien tiene la suite delante, el escenario deja de medir lo que el diseño dijo y el hueco no vuelve nunca al método. \`infra/score-scenarios.sh\` comprueba que el snapshot sigue siendo el que build escribió y sale con \`2\` si no. Comprueba solo dos cosas antes de arrancar: que existe \`specs/validation-scenarios.md\` (sin escenarios no hay contra qué validar el servidor: detente y pide cerrar el diseño con \`/keel-design\`) y que esta raíz es un repo git (si no, \`git init -b main\`). El stack ya lo eligió el diseñador y está en \`keel-stack.json\`: respétalo; para cambiarlo hay que borrarlo y re-ejecutar \`keel-spring build --force\` en el workspace. Solo pregunta al usuario decisiones que el scaffolding no cubre, y regístralas en el \`README.md\`.

   **Modo evolución.** Si existe \`build/keel-refresh/EVOLUTION.md\`, este proyecto ya estaba completado y algo cambió desde entonces —el diseño, el stack o el generador—: léelo entero antes de lanzar nada. No es otra generación sino el mismo pipeline con el trabajo acotado: a \`keel-spring-code\` le pasas la ruta de ese documento (fusiones pendientes, huérfanos a retirar, cambios del diseño, stubs nuevos y stack); a \`keel-spring-tests\`, la ruta y **solo su sección 4** (los \`FL-*\` añadidos, cambiados y quitados: las clases de los demás flujos no se reescriben); \`keel-spring-infra\` hace lo de siempre. La puntuación, el arbitraje y la calidad **no cambian**: \`score-scenarios.sh\` corre la suite completa, y las clases de los flujos que no se tocaron son la no-regresión de lo que ya funcionaba. Hasta cerrar, nadie ejecuta \`./gradlew clean\`: se llevaría el documento, la base del delta y las versiones nuevas de los conflictos.
...
6. **Informe de generación.** Antes del commit, escribe **dos** archivos en esta raíz. (a) \`INFORME-GENERACION.md\`, que **abre con la matriz final de \`infra/score-scenarios.sh\` y su código de salida, literales** (sección 0: sin ella el informe se lee como el de un servicio que pasó el gate aunque no lo pasara), y sigue con lo que apareció durante la generación y **no es de este proyecto**, sino del generador. (b) \`design-gaps.yaml\` con **cada** \`designGap\` —schema \`design-gaps.schema.json\` de keel-core, sellado con el \`service\` y la \`version\` de \`specs/service.keel.yaml\`, una entrada por hueco con \`layer\`, \`unit\`, \`kind\`, \`proposal\`, \`source\` y el \`scenario\` que lo destapó—, también cuando el informe ya los cuente en prosa: la prosa no vuelve sola al diseño, el YAML sí (\`keel-spring check\` y \`/keel-evolve\` lo leen desde el workspace). Sin huecos, no se escribe. Fuentes, todas de los bloques estructurados que ya recibiste: \`harnessPatches\` (parches al andamiaje), los \`failures\` con \`culprit: harness\`, los \`probes[].verdict: FALSO-NEGATIVO\` de infraestructura y los \`designGaps\` consolidados de los cinco agentes. Estructura y regla de oro —cada entrada dice de quién es— en \`{{keel:docs}}/orchestration.md\` § El cierre devuelve al generador lo que es del generador. Si no hubo nada que reportar, dilo en una línea y no inventes hallazgos: un informe vacío es un buen resultado.
7. **Cerrar.** Commit (\`Generado desde specs/${service.name} v${service.version}\`) y resumen: decisiones, matriz escenario → resultado, estado de cada agente, ajustes de calidad aplicados/pendientes y huecos del diseño detectados (\`designGaps\` consolidados). **En modo evolución**, antes del commit: quita de \`pendingMerge\` en \`keel-generated.json\` las rutas que se fusionaron (la clave entera si no queda ninguna) —mientras quede una, \`keel-spring build --check\` sale en rojo, y así debe ser—, borra \`build/keel-refresh/\` entero y añade al resumen el delta aplicado; el commit pasa a ser \`Evolucionado desde specs/${service.name}\` con las dos versiones que nombra \`EVOLUTION.md\`.
