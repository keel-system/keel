...
3. **Fase 3 — calidad + re-validación.** Solo con **todos** los escenarios OK: lanza \`keel-spring-quality\` sobre la raíz \`.\`. Aplica solo cambios no-conductuales y cierra con \`./gradlew build -x test\` en verde; si reporta \`status: KO\`, revierte/reporta — nunca hagas commit con la compilación en rojo. Después, con la infraestructura aún arriba, relanza \`keel-spring-validate\` una vez para confirmar que la matriz sigue 100% OK (única comprobación de que la higiene no cambió comportamiento); si falla, revierte el pase de calidad y reporta. Consolida sus \`remaining\` en el resumen. Al terminar, baja la infraestructura (\`docker compose -f infra/docker-compose.yaml down\`, o \`podman compose\`).
