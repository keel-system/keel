...
- *use-cases*: todo `command` declara al menos un error; commands disparados por subscription con `retry.maxAttempts > 1` sin `idempotency` (**error**); `cache.invalidatedBy` no cubre todos los eventos que mutan lo cacheado —`keel validate` ya marca en rojo el caso del `embed` (la entidad embebida sin ninguna vía) y avisa de los eventos de la entidad principal que falten; lo que queda aquí es lo que ninguna regla mecánica ve: vías que **no** son eventos, como una operación de otro servicio o un cambio de configuración—; command que muta entidades de **dos** agregados en una operación (sugerir evento + consistencia eventual).
