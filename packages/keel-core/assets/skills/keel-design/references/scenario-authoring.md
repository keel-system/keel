...
| `operations[].schedule` | disparo y efecto observable + —si ACTÚA sobre lo que encuentra— el de **clúster**: dos instancias vivas, VARIAS filas candidatas y un `Then` que cuenta el efecto externo **recibido** y afirma una vez por fila. El escenario tiene que **nombrar la operación** |
...
| `messaging.subscriptions` | consumo + `onFailure` (retry/DLQ) + **reentrega del mismo `messageId`** siempre que haya guarda declarada: `contract.messageId`, la envoltura `keel` (que ya trae `metadata.eventId`) o una transición irrepetible. El canal es at-least-once para todas, no solo para las compensatorias |
...
- **Barrido sin escenario de clúster**: `@Scheduled` corre en TODAS las réplicas, así que sin reclamo las N se llevan las mismas filas y cada una actúa. Es el hueco que ningún escenario de una sola instancia puede tapar —con una réplica, reclamar y leer dan el mismo resultado— y el modo de fallo que más se parece a estar bien: la consulta es correcta y lo único que falla es dónde cae el commit.
- **Suscripción con guarda y sin escenario de reentrega**: el mismo hueco fuera de la compensación. La guarda que el diseño declara —o la que hereda de la envoltura `keel` sin decir nada— es una promesa, y lo único que la comprueba es entregar el mismo `messageId` dos veces. Con ids distintos son dos hechos distintos y el escenario pasa contra un consumidor que no deduplica nada.
...
De esta lista, `keel validate` caza seis **por texto**: los dos de compensación (reentrega y doble entrega), la reentrega de cualquier otra suscripción con guarda, el de la carrera de idempotencia, el del outbox y el de clúster de cada barrido que actúa. Busca la señal en los escenarios y avisa si no la encuentra. Si el escenario existe con otra redacción, el aviso es un falso positivo que se cierra escribiendo la palabra que toca («reentrega», «a la vez», «canal indisponible»); si no existe, hay que escribirlo. El último —la llamada de vuelta al proveedor— no se mecaniza: depende de qué encarga cada dependencia, y es revisión de `/keel-validate`.
