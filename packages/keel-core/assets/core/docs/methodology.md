...
4. **Documentar** — del mismo spec se derivan documentaciones complementarias, según a quién sirvan: `/keel-docs` produce `openapi.yaml` y colecciones Postman (`postman/`) para clientes de la **API de usuarios** (web/mobile); `/keel-integrate` produce `INTEGRATION.md`, el **contrato servidor-a-servidor** (endpoints expuestos a otros servidores + eventos publicados/consumidos) para que **otro servidor** lo consuma; `/keel-handoff` produce `DESIGN.md` (características del dominio + decisiones de diseño con su porqué) para que **otro equipo reutilice el diseño** sin leer el código. `DESIGN.md` y el índice del `README.md` se generan automáticamente al cerrar el diseño (paso 1); `/keel-handoff` los **regenera** cuando el spec cambia (re-deriva lo mecánico y preserva el porqué ya capturado).
