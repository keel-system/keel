...
1. **Diseñar** (`/keel-design`) — el agente entrevista al humano sobre el dominio y construye el diseño **capa a capa** (ver "Diseño por capas" más abajo): cada capa es un artefacto YAML propio que el humano aprueba, y **cierra validándose** (`keel validate --wip`, ver paso 2) antes de pasar a la siguiente, revisable en un diff pequeño. Diseñar y validar son el mismo bucle, no fases separadas: se itera hasta que el diseño completo pasa en verde. El **cierre** del diseño produce entonces `specs/<servicio>/validation-scenarios.md` (formato: [validation-scenarios.md](validation-scenarios.md)): escenarios Given/When/Then que cubren toda operación y todo error declarado, el contrato con el que la generación validará el servidor. Como paso final del cierre, el agente ejecuta automáticamente `/keel-handoff` para derivar `docs/<servicio>/DESIGN.md` (capturando en el momento el porqué de las decisiones) y actualizar el índice de servicios del `README.md` del workspace. Esta es **documentación de diseño**: queda lista al cerrar, **antes de generar nada**, de modo que el diseño sea visible y reutilizable por otros equipos apenas se termina.
...
4. **Documentar** — conviene distinguir **dos clases** de documentación, que se producen en momentos distintos:
   ...
   - **Documentación de integradores** — se deriva del mismo spec cuando alguien va a consumir el servicio, tras el cierre e **independiente de que se haya generado código**: `/keel-docs` produce `openapi.yaml` y colecciones Postman (`postman/`) para clientes de la **API de usuarios** (web/mobile); `/keel-integrate` produce `INTEGRATION.md`, el **contrato servidor-a-servidor** (endpoints expuestos a otros servidores + eventos publicados/consumidos) para que **otro servidor** lo consuma.
...
- Antes de un cambio major, ejecutar `/keel-docs` y `/keel-integrate` sobre ambas versiones y comparar los `openapi.yaml` (API de usuarios) y los `INTEGRATION.md` (contrato servidor-a-servidor) para enumerar exactamente qué rompe.
