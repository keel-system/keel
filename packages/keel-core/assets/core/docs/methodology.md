...
```
   ¿existe ya un diseño parecido?  ──sí──> derivarlo (--from registry:<diseño>)
...
0. **Buscar antes de diseñar** (`keel registry`) — el paso más barato del ciclo es el que evita recorrerlo. Un diseño desde cero cuesta una sesión de entrevista capa a capa; uno derivado de otro que ya resuelve el problema cuesta una revisión de lo que cambia. Antes de arrancar, `keel registry search <dominio>` busca en el registry de diseños publicados y `keel registry show <diseño>` da su ficha —capas, contenido, enlaces a su `DESIGN.md` y su panel— para evaluarlo **sin descargarlo**. Si encaja, el paso 1 lo deriva con `--from registry:<diseño>` y `/keel-design` arranca en modo derivación. Un registry es un repositorio git con la forma de un workspace Keel, así que una organización puede tener el suyo (`KEEL_REGISTRY_URL`) con sus diseños internos y el mecanismo es el mismo. Detalle en [design-registry.md](design-registry.md).
