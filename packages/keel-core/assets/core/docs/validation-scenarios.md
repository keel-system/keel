...
- **Si el diseño declara `dependencies: compensations`**, cada compensación tiene dos escenarios, y ninguno es opcional: (1) llega el evento de fallo y se verifica el efecto completo —el trabajo deshecho **y el estado propio devuelto**, leyéndolo por la API—; (2) **el mismo evento se reentrega** y el `Then` afirma que no hay segundo efecto. Una compensación es lo que se ejecuta cuando algo ya salió mal, por un canal que reentrega: es la parte del servicio con menos probabilidad de ejercitarse a mano y más coste si está rota. **Es la única regla de cobertura que comprueba `keel validate`**: busca escenarios que mencionen el `onEvent` de la compensación y, entre ellos, uno que lo reentregue, y avisa si no los encuentra. El resto de esta sección sigue siendo revisión de `/keel-validate`; esta se mecanizó porque el gate del generador solo puntúa lo que este documento declara, así que un escenario que faltaba aquí no lo echaba de menos nadie.
