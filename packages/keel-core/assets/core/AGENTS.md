...
keel registry → keel new → /keel-design (+ /keel-consume si depende de otros servidores;
        ...
        → /keel-validate → keel-<tech> check → keel-<tech> build → cd services/<servicio>-<tech> → /keel-generate-<tech>
        → /keel-docs + /keel-integrate

y después, cada vez que el diseño cambie:  /keel-evolve → keel-<tech> build --refresh --prune
                                           → cd services/<servicio>-<tech> → /keel-generate-<tech>
                                             (modo evolución: solo lo que cambió)
```
...
4. **Validar** — `/keel-validate` (usa `keel validate specs/<servicio>` para schemas por capa + referencias cruzadas, y añade la checklist semántica). Y antes de dar el diseño por cerrado, **pregúntale al generador que vayas a usar si puede con él**: `keel-<tech> check specs/<servicio>` (ver los conocidos con `keel list`) no escribe nada y dice lo que solo se vería al generar — qué del diseño no materializa ese generador, y qué va a dejar como trabajo del agente porque el diseño no lo decide. Un hueco que aparece aquí cuesta una frase; el mismo hueco descubierto dentro del proyecto generado cuesta un ciclo entero de agentes contra infraestructura real. Es del generador y no de `keel validate` a propósito: el núcleo no conoce ninguna tecnología, y la respuesta depende de cuál elijas.
5. **Generar** — siempre **dos pasos**, y en este orden:
   ...
   No hay ninguna skill de generación en este workspace: el generador vive solo dentro del proyecto que produce. Si el diseño cambia, **no se regenera desde cero ni se toca el código primero**: el cambio entra por el diseño (paso 7) y el proyecto se pone al día con `keel-<tech> build specs/<servicio> --refresh --prune`, que nunca pisa el código del agente. `--force` sí lo pisa, y casi nunca es lo que se quiere.
...
7. **Evolucionar** — `/keel-evolve specs/<servicio>`: la puerta única para cambiar un diseño **ya cerrado**. Traduce el cambio a capas afectadas, itera solo esas con `/keel-design`, versiona el contrato (patch/minor/major) y **regenera en cascada todos los derivados** que el cambio deja atrás: `validation-scenarios.md`, `DESIGN.md`, los contratos formales y el panel de `/keel-docs`, e `INTEGRATION.md`. Cada derivado lleva estampado el `service.version` del que nació, y `keel describe <servicio>` los inventaría comparando ese sello con el manifiesto: es lo que hace mecánicamente visible que uno quedó atrás.
   ...
   Si el servicio **ya tiene un servidor generado**, la evolución sigue en él y parte siempre del diseño recién cerrado:
   1. `keel-<tech> build specs/<servicio> --refresh --prune` desde este workspace. Pone al día lo que es del generador y nadie tocó, deja aparte la versión nueva de lo que el agente completó y el generador volvió a cambiar (una **fusión pendiente**), borra lo que el diseño quitó y nadie tocó, y pregunta solo las tecnologías nuevas que el diseño empiece a pedir. Deja en el proyecto un documento de evolución con lo que cambió desde la última vez que se completó.
   2. `cd services/<servicio>-<tech>` y `/keel-generate-<tech>`. Con ese documento, la skill entra en **modo evolución**: el agente de código trabaja sobre lo que cambió y el de pruebas solo sobre los escenarios afectados, pero la puntuación corre la suite completa como no-regresión.

   `keel-<tech> build --check` es la puerta de CI: sale en rojo mientras el servidor no esté al día con su diseño. Referencia: `docs/methodology.md § El servidor también evoluciona desde el diseño`.
