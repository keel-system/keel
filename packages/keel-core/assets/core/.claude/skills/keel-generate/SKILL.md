...
Cada generador es un paquete npm independiente con CLI propia (ej. `keel-springboot`); su comando `build` instala en el workspace la skill `keel-generate-<tech>` y los archivos de `generators/<tech>/`, y valida el diseño. Puedes invocar directamente `/keel-generate-<tech>` si ya sabes la tecnología; este orquestador solo resuelve y verifica antes de delegar.

## Proceso

1. **Resolver el generador.** Comprueba que existen `.claude/skills/keel-generate-<tecnología>/` y `generators/<tecnología>/` en el workspace. Si no, indica al usuario cómo instalarlo: `npm i -g keel-<tecnología>` (ej. `keel-springboot`; ver `keel list`) y `keel-<tecnología> build specs/<servicio>`. No intentes generar sin generador instalado.
