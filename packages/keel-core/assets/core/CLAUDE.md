...
4. **Generar** — `/keel-generate <tech> specs/<servicio>`: delega en el generador instalado en `generators/<tech>/`. Cada generador es un paquete npm con CLI propia: se instala con `npm i -g keel-<tech>` (ej. `keel-springboot`; ver conocidos: `keel list`) y se prepara con `keel-<tech> build specs/<servicio>` (copia su skill y convenciones al workspace, valida el diseño y, si el generador lo soporta, genera el scaffolding determinista del servicio). Salida: `services/<servicio>-<tech>/` como repo git propio.
5. **Documentar** — `/keel-docs specs/<servicio>` deriva `INTEGRATION.md` + `openapi.yaml` para integradores externos. (El documento de diseño `DESIGN.md` ya se produjo al cerrar el diseño; `/keel-handoff specs/<servicio>` lo **regenera** cuando el spec cambia.)
